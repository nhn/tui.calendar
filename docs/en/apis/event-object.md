...
| [isAllday](#isallday)                       | <code>false</code>        | All-day availability                                                                                                                                                                                                    |
| start                                       | <code>new TZDate()</code> | The date and time the event starts. When creating an event, you can specify <code>Date</code>, <code>string</code>, <code>number</code>, or <code>TZDate</code>, and in some calendar APIs' parameter or return value is a <code>TZDate</code> object. |
| end                                         | <code>new TZDate()</code> | The date and time the event starts. When creating an event, you can specify <code>Date</code>, <code>string</code>, <code>number</code>, or <code>TZDate</code>, and in some calendar APIs' parameter or return value is a <code>TZDate</code> object.   |
| goingDuration                               | <code>0</code>            | The travel time takes to get going. It is a number in minutes.                                                                                                                                                        |
| comingDuration                              | <code>0</code>            | The travel time takes to get back. It is a number in minutes.                                                                                                                                                   |
...
| dueDateClass                                | <code>''</code>           | Category for task events. Any string is possible.                                                                                                                                                                       |
...
| [dragBackgroundColor](#style-related-properties)    | <code>'#a1b56c'</code>    | Background color while dragging the event                                                                                                                                                                               |
| [borderColor](#style-related-properties)            | <code>'#000'</code>       | Left border color of the event element                                                                                                                                                                                      |
| [customStyle](#style-related-properties)            | <code>{}</code>           | The style to apply to the calendar element. [A JavaScript object with CSS properties in camelCase](https://reactjs.org/docs/dom-elements.html#style).                                                                           |
| raw                                         | <code>null</code>         | Arbitrary data for the event. You can leverage for your own use cases.                                                                                                                                                                                                  |
...
Events will be rendered following their category. A category is one of `milestone`, `task`, `allday`, or `time`.

### isReadOnly

![isReadOnly](../../assets/EventObject_isReadOnly.png)

Indicates whether the event can be modified. If `isReadOnly` is `true`, you cannot move or resize the event, and the edit button is not exposed in the schedule detail popup.

### isPending, isFocused, isPrivate

`isPending` indicates whether an event is pending or not, `isFocused` indicates whether an event is emphasized or not, and `isPrivate` indicates whether an event is private or not. Basically, it does not affect the rendering, and if you want to display the event differently according to these values, you can use the [template](./template.md) functionality.
