...
function getRelativePosition(position, relativeElement) {
  var clientX = Array.isArray(position) ? position[0] : position.clientX;
  var clientY = Array.isArray(position) ? position[1] : position.clientY;

  if (!relativeElement) {
    return [clientX, clientY];
  }

  var _relativeElement$getB = relativeElement.getBoundingClientRect(),
      left = _relativeElement$getB.left,
      top = _relativeElement$getB.top;
  ...
  matrices.forEach(function (matrix) {
    ...
    matrix.forEach(function (row) {
      row.forEach(function (uiModel, col) {
        ...
        var _uiModel$valueOf = uiModel.valueOf(),
            goingDuration = _uiModel$valueOf.goingDuration,
            comingDuration = _uiModel$valueOf.comingDuration;
...
function HorizontalEvent(_ref4) {
  ...
  var onMoveStart = useDrag(DRAGGING_TYPE_CREATORS.moveEvent('dayGrid', "".concat(uiModel.cid())), {
    onDragStart: function onDragStart() {
      return startDragEvent(classNames.moveEvent);
    },
...
function calculatePopupArrowPosition(eventRect, layoutRect, popupRect) {
  var top = eventRect.top + eventRect.height / 2;
...
function EventFormPopup() {
  var _ref4, _formState$calendarId, _calendars$;
  ...
  var selectedCalendarId = (_ref4 = (_formState$calendarId = formState.calendarId) !== null && _formState$calendarId !== void 0 ? _formState$calendarId : event === null || event === void 0 ? void 0 : event.calendarId) !== null && _ref4 !== void 0 ? _ref4 : (_calendars$ = calendars[0]) === null || _calendars$ === void 0 ? void 0 : _calendars$.id;
  return W(v("div", {
    role: "dialog",
...
function TimeEvent(_ref3) {
  ...
  var onMoveStart = useDrag(DRAGGING_TYPE_CREATORS.moveEvent('timeGrid', "".concat(uiModel.cid())), {
    onDragStart: function onDragStart() {
      return startDragEvent(timeEvent_classNames.moveEvent);
    },
...
function useTimeGridEventResize(_ref) {
  ...
  var baseResizingInfo = F(function () {
    ...
    var startTZDate = resizeTargetUIModelColumns[eventStartDateColumnIndex][0].getStarts();
    var eventStartDateRowIndex = rows.findIndex(findRowIndexOf(startTZDate, eventStartDateColumnIndex)); // when it is -1, the event starts before the current view.
    ...
    var endTZDate = resizeTargetUIModelColumns[eventEndDateColumnIndex][0].getEnds();
    var eventEndDateRowIndex = rows.findIndex(findRowIndexOf(endTZDate, eventEndDateColumnIndex)); // when it is -1, the event ends after the current view.
  ...
  useWhen(function () {
    ...
    if (shouldUpdate) {
      var eventEndDateColumnIndex = baseResizingInfo.eventEndDateColumnIndex,
          eventStartDateRowIndex = baseResizingInfo.eventStartDateRowIndex,
          eventStartDateColumnIndex = baseResizingInfo.eventStartDateColumnIndex;
      var targetEndDate = setTimeStrToDate(timeGridData.columns[columnIndex].date, timeGridData.rows[eventStartDateColumnIndex === eventEndDateColumnIndex ? Math.max(currentGridPos.rowIndex, eventStartDateRowIndex) : currentGridPos.rowIndex].endTime);
      eventBus.fire('beforeUpdateEvent', {
...
function getMovingEventPosition(_ref) {
  ...
  var nextStart = addMilliseconds(draggingEvent.getStarts(), millisecondsDiff);
  var nextEnd = addMilliseconds(draggingEvent.getEnds(), millisecondsDiff);
  ...
  var height = isEndAtNextDate ? maxHeight : indexDiff * rowHeight;
  return {
    top: top,
    height: height
  };
...
function hasGoingDuration(uiModel, options) {
  ...
  var _uiModel$valueOf = uiModel.valueOf(),
      _uiModel$valueOf$goin = _uiModel$valueOf.goingDuration,
      goingDuration = _uiModel$valueOf$goin === void 0 ? 0 : _uiModel$valueOf$goin;

  return goingDuration && startColumnTime <= goingStart;
}

function hasComingDuration(uiModel, options) {
  var comingEnd = options.comingEnd,
      endColumnTime = options.endColumnTime;

  var _uiModel$valueOf2 = uiModel.valueOf(),
      _uiModel$valueOf2$com = _uiModel$valueOf2.comingDuration,
      comingDuration = _uiModel$valueOf2$com === void 0 ? 0 : _uiModel$valueOf2$com;
...
function setRenderInfo(uiModel, columnIndex, baseWidth, startColumnTime, endColumnTime) {
  var _uiModel$valueOf3 = uiModel.valueOf(),
      _uiModel$valueOf3$goi = _uiModel$valueOf3.goingDuration,
      goingDuration = _uiModel$valueOf3$goi === void 0 ? 0 : _uiModel$valueOf3$goi,
      _uiModel$valueOf3$com = _uiModel$valueOf3.comingDuration,
      comingDuration = _uiModel$valueOf3$com === void 0 ? 0 : _uiModel$valueOf3$com;
...
function getSeeMorePopupPosition(position, popupSize, appContainerSize) {
  ...
  var _position = gridCell_slicedToArray(position, 2),
      leftPos = _position[0],
      topPos = _position[1];

  var width = popupSize.width,
      height = popupSize.height;
  var calendarWidth = leftPos * containerWidth / 100;
  var calendarHeight = topPos * containerHeight / 100;
  var isOverWidth = calendarWidth + width >= containerWidth;
  var isOverHeight = calendarHeight + height >= containerHeight;
  var left = toPercent(leftPos);
  var top = toPercent(topPos);
...
function getSeeMorePopupRect(_ref2) {
  ...
  var appContainerSize = getSize(layoutContainer);
  var pos = getRelativePosition({
    clientX: getPosition(cell).x,
    clientY: getPosition(grid).y
  }, layoutContainer);
  var left = pos[0] - MONTH_MORE_VIEW_PADDING;
  var top = pos[1] - MONTH_MORE_VIEW_PADDING;
  left = ratio(appContainerSize.width, 100, left);
  top = ratio(appContainerSize.height, 100, top);
  var popupPosition = getSeeMorePopupPosition([left, top], popupSize, appContainerSize);
