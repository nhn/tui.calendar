...
`Calendar` 클래스를 사용하여 캘린더를 생성할 수 있다. 캘린더를 생성하고 싶은 위치(엘리먼트)를 지정하고, 필요하다면 적절한 옵션을 설정해야 한다.
...
인스턴스 생성 시 두 번째 인자로 옵션을 설정할 수 있다. 별도로 지정하지 않은 옵션은 기본값이 설정된다. 옵션에 대한 자세한 정보는 [TODO Options 문서에서]() 살펴볼 수 있다.
...
인스턴스를 생성한 이후 캘린더 인스턴스의 메서드를 사용하여 캘린더의 동작을 제어할 수 있고, 인스턴스 이벤트에 이벤트 핸들러를 등록할 수 있다.
...
주어진 옵션 값을 기반으로 캘린더 전체의 HTML 문자열을 생성하여 리턴한다. 서버 사이드 렌더링 환경에서 활용할 수 있다.
...
이벤트를 생성할 때 필요한 정보는 [EventObject](/story/documentation-ko-api-eventobject--page) 문서를 참고하라.
...
테마를 변경한다. 적용 가능한 테마는 [테마](/story/documentation-ko-api-테마--page) 문서를 참고하라.
...
- 파라미터
   - `calendars` - 캘린더 정보의 배열. 캘린더 정보는 아래의 타입을 가진다. 자세한 내용은 [캘린더](/story/documentation-ko-api-eventobject--page#캘린더calendarid) 문서에서 확인하라.
...
- 파라미터
  ...
  - `colorOptions` - 적용할 컬러 설정값. 자세한 내용은 [캘린더](/story/documentation-ko-api-eventobject--page#캘린더calendarid) 문서 참고
...
- 파라미터
  - `event` - 이벤트 정보를 가지고 있는 객체. 자세한 내용은 [EventObject](/story/documentation-ko-api-eventobject--page) 문서 참고.
...
| [beforeDeleteEvent](#beforedeleteevent)               | 기본 일정 상세 팝업에서 Delete 버튼을 눌렀을 때 발생                       |
...
| [clickTimezoneCollapseBtn](#clicktimezonecollapsebtn) | 주간 뷰 혹은 일간 뷰에서 여러개의 타임존을 표시한 경우 나타나는 접기 버튼을 클릭할 때 발생    |
...
캘린더 인스턴스 옵션 중 `isReadOnly` 가 `true` 가 아닐 때, 캘린더 영역의 빈 공간을 클릭하거나 드래그 앤 드랍 했을 경우 특정 날짜 혹은 시간을 선택할 수 있다.
...
- 파라미터: `event: EventObject` - 이벤트 생성 팝업에서 입력된 정보가 전달된다. 전달되는 객체의 자세한 정보는 [EventObject](/story/documentation-ko-api-eventobject--page)를 참고하라.

캘린더 인스턴스 옵션 중 `useCreationPopup` 가 `true` 일 때, 기본 이벤트 생성 팝업을 사용할 수 있다.
...
- 파라미터: `updatedEventInfo: UpdatedEventInfo` - 기존 이벤트 정보와 수정된 이벤트 정보를 가지고 있다. 전달되는 객체의 자세한 정보는 [EventObject](/story/documentation-ko-api-eventobject--page)를 참고하라.
...
- 캘린더 인스턴스 옵션 중 `useCreationPopup` 와 `useDetailPopup` 가 모두 `true` 이고, 이벤트 상세 팝업을 통해 이벤트를 수정 후 Update 버튼을 누를 때
...
- 파라미터: `event: EventObject` - 삭제할 이벤트의 정보가 전달된다. 전달되는 객체의 자세한 정보는 [EventObject](/story/documentation-ko-api-eventobject--page)를 참고하라.
...
- 파라미터: `event: EventObject` - 이벤트 정보가 전달된다. 전달되는 객체의 자세한 정보는 [EventObject](/story/documentation-ko-api-eventobject--page)를 참고하라.
...
이벤트 정보의 자세한 내용은 [EventObject](/story/documentation-ko-api-eventobject--page)를 참고하라.
...
캘린더 인스턴스 옵션에서 [TODO 타임존]()을 두 개 이상 사용하고, `week.showTimezoneCollapsedButton` 이 `true` 인 경우
