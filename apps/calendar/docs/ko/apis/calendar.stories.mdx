...
캘린더 인스턴스 생성 시 두 번째 인자로 옵션을 설정할 수 있다. 별도로 지정하지 않은 옵션은 기본값이 설정된다. 옵션에 대한 자세한 정보는 [TODO Options 문서에서]() 살펴볼 수 있다.
...
인스턴스를 생성한 이후 캘린더 [인스턴스의 메서드](#인스턴스-메서드)를 사용하여 캘린더의 동작을 제어할 수 있고, [인스턴스 이벤트](#인스턴스-이벤트)에 이벤트 핸들러를 등록할 수 있다.
...
이벤트를 생성할 때 필요한 정보는 [EventObject](/story/documentation-ko-api-eventobject--page) 문서를 참고한다.
...
캘린더 인스턴스의 테마를 변경한다. 적용 가능한 테마는 [테마](/story/documentation-ko-api-테마--page) 문서를 참고한다.
...
현재 캘린더 인스턴스의 옵션을 변경한다. 각 옵션과 자세한 동작은 [TODO Options]() 문서를 참고한다.
...
캘린더 인스턴스가 현재 렌더링하고 있는 날짜의 범위 중 시작 일시를 리턴한다.
...
- 파라미터: `event: EventObject` - 이벤트 생성 팝업에서 입력된 정보가 전달된다. 전달되는 객체의 자세한 정보는 [EventObject](/story/documentation-ko-api-eventobject--page)를 참고한다.
...
- 파라미터: `updatedEventInfo: UpdatedEventInfo` - 기존 이벤트 정보와 수정된 이벤트 정보를 가지고 있다. 전달되는 객체의 자세한 정보는 [EventObject](/story/documentation-ko-api-eventobject--page)를 참고한다.
...
- 파라미터: `event: EventObject` - 삭제할 이벤트의 정보가 전달된다. 전달되는 객체의 자세한 정보는 [EventObject](/story/documentation-ko-api-eventobject--page)를 참고한다.
...
- 파라미터: `event: EventObject` - 이벤트 정보가 전달된다. 전달되는 객체의 자세한 정보는 [EventObject](/story/documentation-ko-api-eventobject--page)를 참고한다.
...
이벤트 정보의 자세한 내용은 [EventObject](/story/documentation-ko-api-eventobject--page)를 참고한다.
