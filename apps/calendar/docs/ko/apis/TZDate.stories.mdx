...
<Meta title="Documentation/ko/API/TZDate" />
...
console.log(calendar.getEvents('1', 'cal1').start); // TZDate
```
...
| [toCustomDate](#tocustomdate) | TZDate 인스턴스 일시를 CustomDate 객체로 반환한다. (TODO 사용 여부 확인 필요) |
...
- 타입: `toCustomDate(): DateInterface`
- 리턴: `DateInterface` - TZDate 인스턴스 일시를 나타내는 CustomDate 객체

TZDate 인스턴스 일시를 CustomDate 객체로 반환한다. (TODO 사용 여부 확인 필요)
