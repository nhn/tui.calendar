...
| 프로퍼티                              | 설명                                                                                               |
| ------------------------------------- | -------------------------------------------------------------------------------------------------- |
| [defaultView](#defaultview)           | 기본 뷰 타입                                                                                       |
| [useCreationPopup](#usecreationpopup) | 기본으로 제공하는 일정 생성 팝업 사용 여부                                                         |
| [useDetailPopup](#usedetailpopup)     | 기본으로 제공하는 일정 상세 팝업 사용 여부                                                         |
| [isReadOnly](#isreadonly)             | 캘린더 전체의 읽기 전용 여부                                                                       |
| [usageStatistics](#usagestatistics)   | [Google Analytics(GA)](https://analytics.google.com/analytics/web/)를 위한 hostname 수집 허락 여부 |
| [eventFilter](#eventfilter)           | 캘린더 전체의 이벤트 필터링 조건                                                                   |
| [week](#week)                         | 주간/일간뷰 관련 옵션                                                                              |
| [month](#month)                       | 월간뷰 관련 옵션                                                                                   |
| [gridSelection](#gridselection)       | 날짜/시간 선택의 클릭/더블 클릭 가능 여부                                                          |
| [timezone](#timezone)                 | 캘린더에서 사용하는 타임존 정보                                                                    |
| [theme](#theme)                       | [테마](/story/documentation-ko-api-테마--page)                                                     |
| [template](#template)                 | [템플릿](/story/documentation-ko-api-템플릿--page)                                                 |
| [calendars](#calendars)               | 캘린더에서 사용하는 캘린더 목록                                                                    |
...
[TOAST UI 캘린더](https://github.com/nhn/tui.calendar)는 [Google Analytics(GA)](https://analytics.google.com/analytics/web/)를 적용하여 오픈 소스 사용에 대한 통계를 수집하여 전 세계에서 얼마나 널리 사용되는지 확인한다.
이는 프로젝트의 향후 진행을 결정하는 중요한 지표 역할을 한다.
`location.hostname`(예를 들어 "ui.toast.com")을 수집하며 사용량에 대한 통계를 측정하기 위해서만 사용된다.
이 옵션을 `false`로 설정하면 통계 수집을 하지 않는다.
...
캘린더의 날짜/시간 선택의 클릭과 더블 클릭의 가능 여부를 지정한다. 기본값은 `true`이다. `true`나 `false`의 `boolean` 타입으로 지정하면 클릭과 더블 클릭이 둘 모두 가능하거나 불가능하게 만든다.
...
캘린더의 테마를 지정한다. 캘린더 인스턴스 생성 시에만 적용할 수 있으며 `setOptions` 메서드로는 변경이 불가능하다. 대신 `setTheme` 메서드로 변경 가능하며 자세한 내용은 [theme 문서](/story/documentation-ko-api-테마--page)에서 확인할 수 있다.
...
캘린더의 템플릿을 지정한다. 자세한 내용은 [template 문서](/story/documentation-ko-api-템플릿--page)에서 확인할 수 있다.
